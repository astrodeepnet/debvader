...
def SNR(gal_noiseless, sky_background_pixel, band=6, snr_min=5):
    ...
    variance = (
        signal + sky_background_pixel[band]
    )  # for a Poisson process, variance=mean
...
def compute_blendedness_total(img_central, img_others):
    ...
    blnd = 1.0 - np.sum(ic * ic) / np.sum(itot * ic)
    return blnd
...
def compute_deltas_for_most_blended(shift, mag, blendedness):  # (shift_path, mag_path):
    ...
    for i in range(len(shift)):
        for j in range(len(shift[i])):
            deltas_r[i][j] = np.sqrt(
                np.square(shift[i][j][0]) + np.square(shift[i][j][1])
            )
...
def load_vae_conv(path, nb_of_bands, folder=False):
    ...
    #### Build the model
    vae_loaded, Dkl, z = vae_functions.build_vanilla_vae(
        encoder, decoder, full_cov=False, coeff_KL=0
    )
...
def load_vae_full(path, nb_of_bands, folder=False):
    ...
    #### Build the model
    vae_loaded, Dkl = vae_functions.build_vanilla_vae(
        encoder, decoder, full_cov=False, coeff_KL=0
    )
...
def apply_ntimes(func, n, args, verbose=True, timeout=None):
    ...
    return [
        res.get(timeout)
        for res in tqdm(
            multiple_results, desc="# castor.parallel.apply_ntimes", disable=True
        )
    ]
