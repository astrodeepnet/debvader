...
class DeblendField:
    ...
    def __init__(self, net, field_image, cutout_size=59, nb_of_bands=6, epistemic_uncertainty_estimation=False, normalised=False):
        """
        to initialize
    ...
    def get_predicted_field(self, res_deblend=None):
        ...
        if res_deblend is not None:
            ...
            for isolated_galaxy_row in res_deblend:
                ...
                pad_end = self.cutout_size+int((self.field_size-self.cutout_size)/2)

                output_images_mean_padded = np.zeros((self.field_size, self.field_size, self.nb_of_bands))
                output_images_mean_padded[pad_start:pad_end, pad_start:pad_end, :] = output_images_mean
                ...
                output_images_stddev_padded[pad_start:pad_end, pad_start:pad_end, :] = output_images_stddev

                x_pos = isolated_galaxy_row['galaxy_distances_to_center_x'] + isolated_galaxy_row['shifts'][0]
                y_pos = isolated_galaxy_row['galaxy_distances_to_center_y'] + isolated_galaxy_row['shifts'][1]

                for band in range(self.nb_of_bands): # scipy.ndimage.shift is slow so looping over bands is faster
                    ...
                    if self.epistemic_uncertainty_estimation:
                        ...
                        output_images_epistemic_padded[pad_start:pad_end, pad_start:pad_end, :] = np.array(epistemic_uncertainty)
    ...
    def get_deblending_meta_data(self, res_deblend=None):
        ...
        res_deblend_meta['predicted_mean_field']  = predicted_field['predicted_mean_field']
        res_deblend_meta['predicted_stddev_field'] = predicted_field['predicted_stddev_field']
        res_deblend_meta['predicted_epistemic_field'] = predicted_field['predicted_epistemic_field']

        return res_deblend_meta

    def deblend_field(self, galaxy_distances_to_center, cutout_images = None, optimise_positions=False, epistemic_criterion=100., mse_criterion=100., field_image=None):
        """
