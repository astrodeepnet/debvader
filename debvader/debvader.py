...
class DeblendField:
    ...
    def __init__(self, net, field_image, cutout_images=None, cutout_size=59, nb_of_bands=6, epistemic_uncertainty_estimation=False, normalised=False):
        self.net = net 
    ...
    def get_residual_field(self, res_deblend=None):
        ...
        if res_deblend is not None:
            for isolated_galaxy_row in res_deblend:
                ...
                output_images_mean_padded = np.zeros((1, self.field_size, self.field_size, self.nb_of_bands))
                output_images_mean_padded[:,int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),
                                    int((self.field_size - self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),:]=isolated_galaxy_row['output_images_mean']
    ...
    def get_predicted_field(self, res_deblend=None):
        ...
        denoised_field_std = np.zeros((self.field_size, self.field_size, self.nb_of_bands))
        denoised_field_epistemic = np.zeros((self.field_size, self.field_size, self.nb_of_bands))
        ...
        if res_deblend is not None:
            ...
            for isolated_galaxy_row in res_deblend:
                ...
                # First create padded images of the stamps at the size of the field to allow for a simple subtraction.
                output_images_mean_padded = np.zeros((self.field_size, self.field_size, self.nb_of_bands))
                output_images_mean_padded[int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),
                                    int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),:]=output_images_mean

                # Create the corresponding standard deviation image (aleatoric uncertainty).
                output_images_stddev_padded = np.zeros((self.field_size, self.field_size, self.nb_of_bands))
                output_images_stddev_padded[int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),
                                    int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),:]=output_images_stddev
                ...
                for band in range(self.nb_of_bands):
                    ...
                    denoised_field[:, :, band] += scipy.ndimage.shift(output_images_mean_padded[:, :, band], shift=(x_pos, y_pos))
                    denoised_field_std[:, :, band] += scipy.ndimage.shift(output_images_stddev_padded[:, :, band], shift=(x_pos, y_pos))

                    output_images_epistemic_padded = np.zeros((self.field_size,self.field_size,self.nb_of_bands))
                    if self.epistemic_uncertainty_estimation:
                        # Create the corresponding epistemic uncertainty image (aleatoric uncertainty).
                        output_images_epistemic_padded[int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),
                                            int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),:]=np.array(epistemic_uncertainty)
                        denoised_field_epistemic[:, :, band] += scipy.ndimage.shift(output_images_epistemic_padded[:, :, band], shift=(x_pos, y_pos))

                

        predicted_field = {}
        predicted_field['denoised_field'] = denoised_field
        predicted_field['denoised_field_std'] = denoised_field_std
        predicted_field['denoised_field_epistemic'] = denoised_field_epistemic
    ...
    def get_deblending_meta_data(self, res_deblend=None):
        ...
        res_deblend_meta['denoised_field']  = predicted_field['denoised_field']
        res_deblend_meta['denoised_field_std'] = predicted_field['denoised_field_std']
        res_deblend_meta['denoised_field_epistemic'] = predicted_field['denoised_field_epistemic']

        return res_deblend_meta

    def deblend_field(self, galaxy_distances_to_center, field_image=None, cutout_images = None, optimise_positions=False, epistemic_criterion=100., mse_criterion=100.):
        """
        ...
        parameters:
            ...
            normalised: boolean to indicate if images need to be normalised
        """
        field_size = self.field_image.shape[1]
        ...
        if isinstance(cutout_images, np.ndarray):
            output_images_mean, output_images_distribution = deblend(self.net, cutout_images, normalised=self.normalised)
            list_idx = list(range(0, len(output_images_mean)))
        else:
            cutout_images, list_idx = extract_cutouts(field_image, field_size, galaxy_distances_to_center, self.cutout_size, self.nb_of_bands)     
            output_images_mean, output_images_distribution = deblend(self.net, cutout_images[list_idx], normalised=self.normalised)  
        if list_idx==[]:
        ...
        if self.epistemic_uncertainty_estimation:
            ...
            epistemic_uncertainty = list(np.zeros((len(list_idx), self.cutout_size, self.cutout_size, self.nb_of_bands)))
        
        for i,k in enumerate(list_idx):

            # Compute epistemic uncertainty (from the decoder of the deblender)
            if self.epistemic_uncertainty_estimation:
                epistemic_uncertainty.append(np.std(deblend(self.net, np.array([cutout_images[k]]*100), normalised=self.normalised)[0], axis = 0))
                epistemic_uncertainty_normalised = np.sum(epistemic_uncertainty[i][:, :, 2]) / np.sum(output_images_mean[i,:,:,2])
            else:
            ...
            if optimise_positions: # TODO: Check if this is giving funny results
                output_images_mean_padded = np.zeros((self.field_size, self.field_size, self.nb_of_bands))
                output_images_mean_padded[int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),
                                    int((self.field_size-self.cutout_size)/2):self.cutout_size+int((self.field_size-self.cutout_size)/2),:]=output_images_mean[i]
                shift_x, shift_y = position_optimization(field_image[0], output_images_mean_padded, galaxy_distances_to_center[k], method="scipy-minimize")
        ...
        res_deblend['output_images_stddev']=list(output_images_distribution.stddev().numpy())
        ...
        self.res_deblend = Table(res_deblend)

        return self.res_deblend



    def iterative_deblending(self, galaxy_distances_to_center=None, cutout_images=None, optimise_positions=False, epistemic_criterion=100., mse_criterion=100.):
        '''
        ...
        while (len(res_step['shifts'])>len(shifts_previous)):
            ...
            self.mse += [metrics.mean_squared_error(prev_residual_field, new_residual_field)] #todo: prev_residual or original field?
            ...
            res_deblend = vstack([res_deblend, res_step])
            k+=1

            print(f'{sum(self.nb_of_deblended_galaxies)} galaxies found up to this step.')
            print(f'deta_mse = {diff_mse}, mse_iteration = '+str(self.mse[-1])+' and mse_previous_step = '+str(self.mse[-2]))
    ...
    def deblending_step(self, field_image, cutout_images = None, optimise_positions=False, epistemic_criterion=100., mse_criterion=100.):
        '''
        ...
        res_step = self.deblend_field(field_image=field_image, galaxy_distances_to_center=detection_k, cutout_images = cutout_images, optimise_positions=optimise_positions, epistemic_criterion=epistemic_criterion, mse_criterion=mse_criterion)
